function [theta1, theta2, theta3] = solve_thetas3(x, y, z)
    ...
    % Check if the file exists and load the initial guess from it
    % Check if the file exists and load the initial guess from it
    result_file = 'previous_results.mat'
    if isfile(result_file)
        load(result_file, 'initial_guess');
    else
        % Conditional initial guess
        if y >= 0
            initial_guess = [pi/2, -pi/2, 0];
        else
            initial_guess = [-pi/2, pi/2, 0];
        end
    end
    ...
    save(result_file, 'initial_guess');
